...
Browser extension captures browsing events.

Sends URLs to the Java server via a local communication port.
...
The popup UI displays the result instantly.
...
Open your browser → Extensions → Developer Mode: ON

Click Load unpacked → Select the Browser-Extension-main/ folder
...
The extension sends example.com → CommunicationServer

Server analyzes via URLAnalyzer

If flagged malicious → blocked.html is shown
